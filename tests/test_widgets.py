...
class WidgetTests(TestCase):
    ...
    def test_check_widget_html(self) -> None:
        widget = widgets.S3UploadWidget(dest="foo")
        html = widget.render("filename", "")
        self.assertEqual(html, HTML_OUTPUT)

    def test_check_signing_logged_in(self) -> None:
        self.client.login(username="admin", password="admin")
        data = {"dest": "files", "name": "image.jpg", "type": "image/jpeg"}
        response = self.client.post(reverse("s3upload"), data)
        self.assertEqual(response.status_code, 200)

    def test_check_signing_logged_out(self) -> None:
        data = {"dest": "files", "name": "image.jpg", "type": "image/jpeg"}
        response = self.client.post(reverse("s3upload"), data)
        self.assertEqual(response.status_code, 403)

    def test_check_allowed_type(self) -> None:
        data = {"dest": "imgs", "name": "image.jpg", "type": "image/jpeg"}
        response = self.client.post(reverse("s3upload"), data)
        self.assertEqual(response.status_code, 200)

    def test_check_disallowed_type(self) -> None:
        data = {"dest": "imgs", "name": "image.mp4", "type": "video/mp4"}
        response = self.client.post(reverse("s3upload"), data)
        self.assertEqual(response.status_code, 400)

    def test_check_allowed_type_logged_in(self) -> None:
        self.client.login(username="admin", password="admin")
        data = {"dest": "vids", "name": "video.mp4", "type": "video/mp4"}
        response = self.client.post(reverse("s3upload"), data)
        self.assertEqual(response.status_code, 200)

    def test_check_disallowed_type_logged_out(self) -> None:
        data = {"dest": "vids", "name": "video.mp4", "type": "video/mp4"}
        response = self.client.post(reverse("s3upload"), data)
        self.assertEqual(response.status_code, 403)

    def test_check_disallowed_extensions(self) -> None:
        data = {"dest": "imgs", "name": "image.jfif", "type": "image/jpeg"}
        response = self.client.post(reverse("s3upload"), data)
        self.assertEqual(response.status_code, 415)

    def test_check_allowed_extensions(self) -> None:
        data = {"dest": "imgs", "name": "image.jpg", "type": "image/jpeg"}
        response = self.client.post(reverse("s3upload"), data)
        self.assertEqual(response.status_code, 200)

    def test_check_disallowed_extensions__uppercase(self) -> None:
        data = {"dest": "imgs", "name": "image.JFIF", "type": "image/jpeg"}
        response = self.client.post(reverse("s3upload"), data)
        self.assertEqual(response.status_code, 415)

    def test_check_allowed_extensions__uppercase(self) -> None:
        data = {"dest": "imgs", "name": "image.JPG", "type": "image/jpeg"}
        response = self.client.post(reverse("s3upload"), data)
        self.assertEqual(response.status_code, 200)

    def test_check_signing_fields(self) -> None:
        self.client.login(username="admin", password="admin")
        data = {"dest": "imgs", "name": "image.jpg", "type": "image/jpeg"}
        response = self.client.post(reverse("s3upload"), data)
    ...
    def test_check_signing_fields_unique_filename(self) -> None:
    ...
    def test_check_policy_conditions(self) -> None:
        self.client.login(username="admin", password="admin")
